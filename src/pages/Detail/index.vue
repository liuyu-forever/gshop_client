        ...
        <div class="InfoWrap">
          <div class="goodsDetail">
            ...
            <div class="priceArea">
              ...
              <div class="couponWrap">
                ...
                <div class="fl fix-width">
                  <div class="fl fix-width">
                    <a
                      href="javascript:;"
                      :title="valueCouponInfo.couponName"
                      v-for="valueCouponInfo in couponInfoList"
                      :key="valueCouponInfo.id"
                    >
                      <i
                        class="red-bg i-coupon"
                        v-if="valueCouponInfo.couponType === 'CASH'"
                        @click="showAsideRight"
                        >现金卷{{ valueCouponInfo.benefitAmount }}元</i
                      >
                      <i
                        class="red-bg i-coupon"
                        v-if="valueCouponInfo.couponType === 'FULL_REDUCTION'"
                        @click="showAsideRight"
                      ...
                      <i
                        class="red-bg i-coupon"
                        v-if="valueCouponInfo.couponType === 'FULL_DISCOUNT'"
                        @click="showAsideRight"
                        >折扣卷{{ valueCouponInfo.benefitDiscount }}</i
                      >
                    </a>
                  </div>
                </div>
              ...
              <div class="activityWrap">
                ...
                <div class="fl fix-width">
                  <div
                    v-for="valueActivity in activityRuleList"
                  ...
                  >
                    ...
                    <em
                      class="t-gray"
                      v-if="valueActivity.activityType === 'FULL_REDUCTION'"
                    >
                    ...
                    <em
                      class="t-gray"
                      v-else-if="valueActivity.activityType === 'FULL_DISCOUNT'"
                    >
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(value, index) in spuSaleAttrList" :key="value.id">
                <dt class="title">{{ value.saleAttrName }}</dt>
                <dd
                  changepirce="0"
                  v-for="valueAttr in value.spuSaleAttrValueList"
                  :key="valueAttr.id"
                  :class="{ active: valueAttr.isChecked === '1' }"
                  @click="setVal(valueAttr, value, index)"
                >
                  {{ valueAttr.saleAttrValueName }}
                  <span
                    title="点击取消选择"
                    :class="{ cancel: valueAttr.isChecked === '1' }"
                  ></span>
                </dd>
              </dl>
            </div>
                    ...
                    <ul style="overflow: auto;">
                      ...
                      >
                        <div class="coupon-price">
                          ...
                          <!-- <span class="coupon-name">满减卷</span> -->
                        </div>
                        <div class="coupon-info">
                          ...
                          <span
                            class="condition"
                            v-if="valueCouponInfo.couponType === 'DISCOUNT'"
                            >折扣券{{ valueCouponInfo.benefitDiscount }}折</span
                          >
                          <!-- <span class="condition">满20000减200元</span> -->

                          <p>可购买：{{ valueCouponInfo.rangeDesc }}</p>
                        </div>
                        <p class="coupon-time">
                          过期时间：{{ valueCouponInfo.expireTime }}
                        </p>
                        ...
                        <div
                          ...
                          v-if="
                            valueCouponInfo.isGet > 0 &&
                              valueCouponInfo.couponStatus === 'USED'
                          "
                        ></div>
                        <button
                          type="button"
                      ...
                      >
                        <div class="coupon-price">
                          <span class="token">150元</span>
                          <span class="coupon-name">满减卷</span>
                        </div>
                        <div class="coupon-info">
                          <span class="condition">满10000减150元</span>
                          <p>可购买：小米10,：Redmi 10X</p>
                        </div>
                        <p class="coupon-time">过期时间：2023-10-19 00:00:00</p>
                        <button
                          type="button"
                          class="sui-btn btn-primary btn-large hm-action-statistics"
                        >
...
export default {
  ...
  /* mounted() {
    this.spuSaleAttrList.forEach(valueAttr => {
      valueAttr.spuSaleAttrValueList.forEach(value => {
        if (value.isChecked === "1") {
          this.selectedSpuSaleAttrValueList.push(value.id);
        }
      });
    });
  ...
  methods: {
    ...
    //点击销售属性显示被点击效果
    setVal(valueAttr, value, index) {
      //遍历当前的属性值对象所在的数组
      value.spuSaleAttrValueList.forEach(value => {
        value.isChecked = "0";
      });
      //给当前点击的属性值添加点击效果
      valueAttr.isChecked = "1";
      //获取当前点击的属性值对应的id
      const valId = valueAttr.id;
    ...
    //领取优惠券
    async receiveCouponInfo(valueCouponInfoId, valueCouponInfo) {
      //获取当前的skuId
      const skuId = this.$route.params.skuId;
      if (!this.$store.state.user.userInfo.name) {
        //如果未登录,则跳转到登录页面,同时传入一个人query参数,保存当前的地址信息
        this.$router.replace({
          path: "/login",
          query: { redirect: `/detail/${skuId}` },
        });
      } else {
        //如果已经登录,调用领券的接口
        try {
          await this.$API.reqGetCouponInfo(valueCouponInfoId);
          await this.$store.dispatch("getCouponActivityList", skuId);
          //表示优惠券没使用
          valueCouponInfo.couponStatus = "NOT_USED";
        } catch (error) {
          this.$message.error(error.message || "领券失败了");
        }
      }
    },
    ...
    async addToCart() {
      ...
      try {
        //分发actoins将商品添加到购物车
        await this.$store.dispatch("getAddToCart", { skuId, skuNum });
        //跳转到AddCartSuccess组件
  ...
  watch: {
    ...
    //监视销售属性数组数据
    spuSaleAttrList: {
      handler(oldSpuSaleAttrList) {
        this.selectedSpuSaleAttrValueList = [];
        oldSpuSaleAttrList.forEach(valueAttr => {
          valueAttr.spuSaleAttrValueList.forEach(value => {
            if (value.isChecked === "1") {
              this.selectedSpuSaleAttrValueList.push(value.id);
            }
