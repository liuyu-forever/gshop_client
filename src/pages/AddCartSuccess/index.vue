      ...
      <div class="goods">
        ...
        <div class="right-gocart">
          <a
            ...
            @click="checkShopDetail"
            >查看商品详情</a
          >
          <a href="javascript:" @click="goShopCartAccounts">去购物车结算 > </a>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  beforeRouteEnter(to, from, next) {
    next(vm => {
      //进入此组件必须携带缓存数据和skuNum,否则跳转到search组件
      const skuInfo = JSON.parse(sessionStorage.getItem("SKUINFO_KEY"));
  ...
  methods: {
    //点击返回到商品详情页面
    checkShopDetail() {
      this.$router.replace(`/detail/${this.skuInfo.id}`);
    },
    //跳转到去购物车计算页面
    goShopCartAccounts() {
      //清空缓存数据
      sessionStorage.removeItem("SKUINFO_KEY")
